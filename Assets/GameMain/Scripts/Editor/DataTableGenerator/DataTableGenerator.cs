...
namespace RoundHero.Editor.DataTableTools
{
    public sealed class DataTableGenerator
    {
        ...
        private static void DataTableCodeGenerator(DataTableProcessor dataTableProcessor, StringBuilder codeContent, object userData)
        {
            ...
            codeContent.Replace("__DATA_TABLE_NAME_SPACE__", "RoundHero");
        ...
        private static string GenerateDataTableParser(DataTableProcessor dataTableProcessor)
        {
                ...
                else
                {
                    ...
                    if (dataTableProcessor.IsEnumrColumn(i))
                    {
                        stringBuilder.AppendLine(
                            $"\t\t\t{dataTableProcessor.GetName(i)} = Enum.Parse<{dataTableProcessor.GetLanguageKeyword(i)}>(columnStrings[index++]);");
                        continue;
                    }
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else if(dataTableProcessor.IsEnumrColumn(i))
                {
                    stringBuilder.AppendFormat("                    {0} = Enum.Parse<{1}>(binaryReader.ReadString());", dataTableProcessor.GetName(i), dataTableProcessor.GetType(i).Name).AppendLine();
                }
                else
                {
