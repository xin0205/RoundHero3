    ...
    public sealed partial class DataTableProcessor
    {
        ...
        public bool SetCodeTemplate(string codeTemplateFileName, Encoding encoding)
        {
            try
            {
                ...
                Debug.Log(Utility.Text.Format("SetValue code template '{0}' success.", codeTemplateFileName));
                return true;
            }
            catch (Exception exception)
            {
                Debug.LogError(Utility.Text.Format("SetValue code template '{0}' failure, exception is '{1}'.", codeTemplateFileName, exception));
                return false;
            }
        }
