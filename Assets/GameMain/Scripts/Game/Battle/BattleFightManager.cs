        ...
        private void CacheUnitActiveMoveDatas(int actionUnitIdx, int effectGridPosIdx, BuffData buffData, ActionData actionData, TriggerData triggerData)
        {
            ...
            else if(Constant.Battle.RelatedUnitFlyRanges.Contains(buffData.FlyRange) || Constant.Battle.DynamicRelatedUnitFlyRanges.Contains(buffData.FlyRange))
            {
                ...
                foreach (var relatedUnit in relatedUnits)
                {
                    ...
                    flyPaths = GetFlyPaths(relatedUnit.GridPosIdx, flyDirect, 1, EUnitActionState.Fly);
            ...
            else if (buffData.FlyRange == EActionType.LineExtend)
            {
                ...
                flyPaths = GetFlyPaths(effectUnit.GridPosIdx, flyDirect, 1, EUnitActionState.Fly);
                    ...
                    if (triggerData.OwnUnitIdx > 0 && triggerData.OwnUnitIdx == triggerData.ActionUnitIdx)
                    {
                        ...
                        if (ownUnit != null && triggerData.UnitStateDetail.UnitState == EUnitState.Empty)
                        {
                            ...
                            if (subHPAddSelfHPCount > 0)
                            {
                                ...
                                var subHPAddSelfHPCountTriggerData = BattleFightManager.Instance.BattleRoleAttribute(actionUnitData.Idx,
                                    actionUnitData.Idx, actionUnitData.Idx, EUnitAttribute.HP, subHPAddSelfHPCount,
                                    ETriggerDataSubType.Unit);
                                SimulateTriggerData(subHPAddSelfHPCountTriggerData, triggerDatas);
                                triggerDatas.Add(subHPAddSelfHPCountTriggerData);
                            }
                        }
