    ...
    public class BattleFightManager : Singleton<BattleFightManager>
    {
        ...
        private void CacheUseCardTrigger()
        {
            foreach (var kv in BattleUnitDatas)
            {
                ...
                {
                    var actionData = new ActionData();
                    actionData.ActionUnitID = kv.Value.Idx;
        ...
        private void CacheUnitActiveMoveDatas(int actionUnitID, int effectGridPosIdx, BuffData buffData, ActionData actionData)
        {
            ...
            else
            {
                ...
                CacheUnitMoveDatas(effectUnit.Idx, flyPaths, moveActionDatas);

                if (moveActionDatas.Count > 0)
                {
                    actionData.MoveData.MoveUnitDatas.Add(moveUnitID, new MoveUnitData()
                    {
                        UnitID = moveUnitID,
                        MoveActionData = moveActionDatas[moveUnitID],
                        UnitActionState = EUnitActionState.Fly,
                    });
            ...
            switch (buffData.FlyType)
            {
                ...
                case EFlyType.SelfPass:
                    ...
                    CacheUnitMoveDatas(actionUnit.Idx, flyPaths, moveActionDatas);
                    if (moveActionDatas.Count > 0)
                    {
                        actionData.MoveData.MoveUnitDatas.Add(actionUnit.Idx, new MoveUnitData()
                        {
        ...
        private void CacheSoliderAttackDatas()
        {
            ...
            foreach (var kv in BattleUnitDatas)
            {
                ...
                var actionData = new ActionData();
                actionData.ActionUnitID = soliderData.Idx;
        ...
        private void CacheSoliderActiveAttackDatas()
        {
            ...
            var actionData = new ActionData();
            actionData.ActionUnitID = unitData.Idx;
                    ...
                    if (passUnitTriggerBuffDatas != null)
                    {
                        foreach (var triggerBuffData in passUnitTriggerBuffDatas)
                        {
                            BattleBuffManager.Instance.BuffTrigger(EBuffTriggerType.Pass,
                                triggerBuffData.BuffData, triggerBuffData.ValueList, unitID, unitID, bePassUnit.Idx,
        ...
        {
            var effectUnitData = GetUnitByID(triggerData.EffectUnitIdx);
            if (effectUnitData == null)
                return;
            
            var actionUnitData = GetUnitByID(triggerData.ActionUnitIdx);
            ...
            if (triggerValue < 0)
            {
                ...
                {
                    if (triggerData.OwnUnitID > 0 && triggerData.OwnUnitID == triggerData.ActionUnitIdx)
                    {
        ...
        {
            var effectUnit = GetUnitByID(triggerData.EffectUnitIdx);
        ...
        public List<Data_BattleUnit> GetUnitByGridPosIdx(int gridPosIdx, EActionType actionType)
        {
            var coord = GameUtility.GridPosIdxToCoord(gridPosIdx);
            var pointList = Constant.Battle.ActionTypePoints[actionType];
                ...
                case ETriggerDataType.Card:
                    switch (triggerData.CardTriggerType)
                    {
                        ...
                        case ECardTriggerType.ToHand:
                            var actionUnit = GetUnitByID(triggerData.ActionUnitIdx);
        ...
        public void UnitAttack(Dictionary<int, ActionData> unitAttackDatas, EActionProgress actionProgress)
        {
            ...
            {
                var effectUnitEntity = BattleUnitManager.Instance.GetUnitByIdx(kv.Value.UnitID);
        ...
        public void CalculateHeroHPDelta(int actionUnitID, Dictionary<int, List<TriggerData>> triggerDatas, bool isMoveTriggerData = false)
        {
            ...
            foreach (var kv in triggerDatas)
            {
                ...
                foreach (var triggerData in kv.Value)
                {
                    ...
                    var unit = GameUtility.GetUnitDataByID(triggerData.EffectUnitIdx, true);
                    if(unit == null)
                        continue;
                    ...
                    hpDeltaDict[unit.UnitCamp].Key = isMoveTriggerData ? kv.Key : playerData.BattleHero.Idx;
                }
        ...
        public void MoveEffectAction(EUnitActionState unitActionState, MoveActionData moveActionData, int moveIdx, int moveUnitID)
        {
            ...
            foreach (var triggerData in triggerDatas)
            {
                ...
                if (triggerData.ActionUnitIdx != -1 && triggerData.EffectUnitIdx != triggerData.ActionUnitIdx)
                {
            ...
            foreach (var triggerTarget in buffData.TriggerTargets)
            {
                switch (triggerTarget)
                {
                    ...
                    case ETriggerTarget.Hero:
                        realEffectUnitIDs.Add(PlayerData.BattleHero.Idx);
                        break;
                    case ETriggerTarget.All:
                        foreach (var kv in BattleFightManager.Instance.RoundFightData.GamePlayData.BattleData.BattleUnitDatas)
                        {
                            ...
                            {
                                realEffectUnitIDs.Add(kv.Value.Idx);
                            }
                    ...
                    case ETriggerTarget.InRange:
                        ...
                        foreach (var gridPosIdx in range)
                        {
                            ...
                            if (unit != null)
                            {
                                realEffectUnitIDs.Add(unit.Idx);
                            }
                        }
                        break;
        ...
        {
            var unit = BattleFightManager.Instance.GetUnitByID(triggerData.ActionUnitIdx);
        ...
        public void  CalculateUnitPaths(EUnitCamp unitCamp, List<int> actionUnitIdxs, List<int> obstacleEnemies, Dictionary<int, List<int>> movePaths)
        {
            ...
            {
                var unit1 = GetUnitByID(actionUnitIdx1);
                var unit2 = GetUnitByID(actionUnitIdx2);
        ...
        {
            return Constant.Battle.CollisionHurt;
        }
