...
namespace RoundHero
{
    ...
    public class BattleFightManager : Singleton<BattleFightManager>
    {
        ...
        private void CacheUnitActiveMoveDatas(int actionUnitIdx, int effectGridPosIdx, BuffData buffData, ActionData actionData)
        {
            ...
            else if(Constant.Battle.RelatedUnitFlyRanges.Contains(buffData.FlyRange) || Constant.Battle.DynamicRelatedUnitFlyRanges.Contains(buffData.FlyRange))
            {
                ...
                foreach (var relatedUnit in relatedUnits)
                {
                    ...
                    CacheUnitMoveDatas(relatedUnit.Idx, flyPaths, moveActionDatas, true);
        ...
        public List<int> CacheUnitMoveDatas(int unitID, List<int> movePaths,
            Dictionary<int, MoveActionData> unitMoveDatas, bool isFly)
        {
            var passUnit = GetUnitByIdx(unitID);
            if (passUnit == null)
                return null;
            if (passUnit.CurHP <= 0)
                return null;
            ...
            if (minFullPaths.Count > 0)
            {
                ...
                RefreshUnitGridPosIdx();
                if (passUnit.UnitCamp == EUnitCamp.Enemy && !isFly)
                {
        ...
        private void CacheUnitMoveDatas(Dictionary<int, List<int>> movePaths, Dictionary<int, MoveActionData> moveDatas)
        {
            ...
            for (int i = 0; i < movePaths.Count; i++)
            {
                movePaths[keys[i]] = CacheUnitMoveDatas(keys[i],
                    movePaths[keys[i]], moveDatas, false);
            }
        ...
        private void CurHPTriggerData(TriggerData triggerData, List<TriggerData> triggerDatas)
        {
            ...
            if (triggerValue < 0)
            {
                ...
                else
                {
                    ...
                    {
                        var ownUnit = GetUnitByIdx(triggerData.OwnUnitID);
        ...
        public List<int> GetMovePaths(int moveUnitIdx, int actionUnitIdx = -1)
        {
            ...
            if(RoundFightData.EnemyMovePaths.ContainsKey(moveUnitIdx))
            {
                ...
                if(RoundFightData.EnemyMovePaths[moveUnitIdx][0] != uniData.GridPosIdx)
                {
        ...
        public List<int> GetEffectUnitIDs(BuffData buffData, int ownUnitID, int actionUnitID, int effectUnitID, int actionUnitGridPosIdx, int actionUnitPreGridPosIdx)
        {
            ...
            var actionUnit = BattleFightManager.Instance.GetUnitByIdx(actionUnitID);
        ...
        public void  CalculateUnitPaths(EUnitCamp unitCamp, List<int> actionUnitIdxs, List<int> obstacleEnemies, Dictionary<int, List<int>> movePaths)
        {
            ...
            foreach (var actionUnitIdx in actionUnitIdxs)
            {
                ...
                movePaths[actionUnitIdx] = CacheUnitMoveDatas(actionUnitIdx,
                    movePaths[actionUnitIdx], unitCamp == EUnitCamp.Enemy ? RoundFightData.EnemyMoveDatas : RoundFightData.ThirdUnitMoveDatas, false);
