	...
	public class FileWritingHandler : System.IDisposable
	{
		...
		public void Dispose()
		{
			...
			if (_handle >= 0)
			{
				...
				NativePlugin.RenderThreadEvent(NativePlugin.PluginEvent.FreeResources, _handle);
			...
			if( _updateMediaGallery )
			{
				...
				{
					// Update the gallery for the images
					System.IO.FileInfo[] aFiles = new System.IO.DirectoryInfo( System.IO.Path.GetDirectoryName( _path ) ).GetFiles();
					foreach( System.IO.FileInfo fileInfo in aFiles )
					{
						CaptureBase.UpdateMediaGallery( fileInfo.FullName );
					}
